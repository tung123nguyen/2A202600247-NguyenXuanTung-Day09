...
def _estimate_confidence(chunks: list, answer: str, policy_result: dict) -> float:
    ...
    # Average của top-3 chunks có score cao nhất (loại bỏ noise từ chunk rank thấp)
    top_chunks = sorted(chunks, key=lambda c: c.get("score", 0), reverse=True)[:3]
    avg_score = sum(c.get("score", 0) for c in top_chunks) / len(top_chunks)

    # Bonus nếu detect được exceptions — tức là system hiểu đúng context phức tạp
    exception_bonus = 0.03 * min(len(policy_result.get("exceptions_found", [])), 2)

    confidence = min(0.95, avg_score + exception_bonus)
